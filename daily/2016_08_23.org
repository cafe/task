...
Also updated =tParaNew()= to make sure all the s_arc values fall on [0.0, 1.0]. Note that sometimes the numerical computation will give results like 0.20000000000000018 or -0.00000000000000018, which actually should be 0.2 or 0.0. So we need to handle such situations in =tParaNew()=:
